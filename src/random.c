...
int randomNumber(bignum bigIntArr, int chunks,bignum n){  //A chunk is 32 bits
    ...
    ULONGLONG targetTime = tickCount + time; 
    ...
    while(chunkWriteCount<=chunks){
        ...
        if(((LONGLONG) targetTime - (LONGLONG) tickCount) <= time*((float)(chunks-chunkWriteCount)/(chunks+1))){ //+1 means chunks aren't written at start or end
