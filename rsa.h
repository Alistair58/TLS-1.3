...
bool isPrime(unsigned long *x,int lenX){ 
    for(int i=0;i<30;i++){ //as MR is incorrect 1/4 of time, it is now incorrect 1 in 4^30 times (once every 36558901 years if it runs every ms)
        unsigned long *a = randomNumber(a,lenX,NULL); //Fermats little theorem only works for 1<a<n-1
        if(!millerRabin(x,a)){
            return false;
        }
        free(a);
...
bool millerRabin(unsigned long *x,int lenX,unsigned long *a,int lenA){
    ...
    if pow(a,exp,n) == 1: #if a^(n-1/2^k) == 1 mod n  -> if a^(n-1/2^k) -1 is a multiple of n, then n is likely prime - using pow is more time efficient
        return True
    while exp < n-1:
